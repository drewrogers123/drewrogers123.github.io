...
function processDiscGolfData(data) {
    ...
    Object.keys(roundGroups).sort((a, b) => parseInt(a) - parseInt(b)).forEach(seq => {
        ...
        for (let i = 0; i < n; i++) {
            ...
            new_R[i] = R[i] + delta;
            console.log(`Player ${players[i]}: R=${R[i]}, S=${S[i]}, E=${E[i]}, delta=${delta}, new_rating=${new_R[i]}`);
        }
    ...
    // Create snapshots with actual round names
    const snapshots = Object.keys(roundGroups).sort((a, b) => parseInt(a) - parseInt(b)).map(seq => {
        const group = roundGroups[seq];
        const roundId = group[0].round_id;
        const roundType = roundId.split('-')[0].trim();
        return {
            ...
            ratings: Object.fromEntries(
                Object.entries(ratings).map(([player, rating]) => [player, Math.round(rating)])
            )
        };
    });
...
function displayResults(results) {
    ...
    // Display snapshots with actual round names
    ...
    if (snapshotsTable && results.snapshots && results.snapshots.length > 0) {
        ...
        if (tbody) {
            tbody.innerHTML = '';
            // Show the final ratings as the last snapshot
            const finalSnapshot = results.snapshots[results.snapshots.length - 1];
            Object.entries(finalSnapshot.ratings)
                .sort(([,a], [,b]) => b - a)
                .slice(0, 10) // Top 10
                .forEach(([player, rating], index) => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>${player}</td>
                        <td>${rating}</td>
                        <td>After ${finalSnapshot.round_id}</td>
                    `;
                    tbody.appendChild(row);
                });
        }
