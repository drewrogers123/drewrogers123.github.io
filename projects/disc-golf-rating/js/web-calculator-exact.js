// Disc Golf Rating Calculator - EXACT Python Implementation
...
function processDiscGolfData(data) {
    console.log('Processing disc golf data with EXACT Python implementation...');
    ...
    // Elo calculation parameters - EXACTLY like Python
    ...
    // Process each round - EXACTLY like Python
    Object.keys(roundGroups).sort((a, b) => parseInt(a) - parseInt(b)).forEach(seq => {
        ...
        // Round type detection - EXACTLY like Python round_type_from_id
        const rtype = roundTypeFromId(roundId);
        const kRound = BASE_K * (WEIGHTS[rtype] || 1.0);

        console.log(`Processing round: ${roundId}, type: ${rtype}, kRound: ${kRound}`);
        ...
        if (n <= 1) {
            // No-op round - add snapshot like Python
            snapshots.push({
                round_id: roundId,
                round_type: rtype,
                round_seq: parseInt(seq),
                ratings: Object.fromEntries(Object.entries(ratings).map(([p, r]) => [p, r])), // Keep as float
                label: `[SNAPSHOT] After ${rtype} — ${roundId}`
            });
        ...
        // Create arrays for calculations - EXACTLY like Python
        const R = players.map(player => ratings[player]);
        const S = new Array(n).fill(0);
        const E = new Array(n).fill(0);
        const W_total = new Array(n).fill(0);

        // Calculate expected scores and actual scores for each pair - EXACTLY like Python
        for (let i = 0; i < n; i++) {
            for (let j = 0; j < n; j++) {
                if (i === j) continue;
                ...
                // Lower score is better - EXACTLY like Python
                let s_ij;
                let margin;
                if (scores[i] < scores[j]) {
                    ...
                    s_ij = 0.0;
                    margin = (scores[i] - scores[j]) * -1.0; // Note: Python has this line
                } else {
                    s_ij = 0.5;
                    margin = 0.0;
                }
                ...
                if (USE_MOV) {
                    const w_ij = movMultiplier(Math.max(0.0, margin), R[i] - R[j]);
                    W_total[i] += w_ij;
                } else {
                    W_total[i] += 1.0;
                }
        ...
        // Calculate new ratings - EXACTLY like Python
        const new_R = [...R];
        for (let i = 0; i < n; i++) {
            const mov_w = USE_MOV ? (W_total[i] / (n - 1)) : 1.0;
            const delta = kRound * mov_w * (S[i] - E[i]);
            new_R[i] = R[i] + delta;
            console.log(`Player ${players[i]}: R=${R[i]}, S=${S[i]}, E=${E[i]}, mov_w=${mov_w}, delta=${delta}, new_rating=${new_R[i]}`);
        }

        // Update ratings and history - EXACTLY like Python
        players.forEach((player, idx) => {
        ...
        // Add snapshot - EXACTLY like Python
        snapshots.push({
            round_id: roundId,
            round_type: rtype,
            round_seq: parseInt(seq),
    ...
    // Create final results - keep ratings as floats like Python
    const finalRatings = Object.entries(ratings)
        .map(([player, rating]) => ({
            player,
            rating: rating, // Keep as float, don't round
            games: 1
        }))
    ...
    const history = historyRows
        ...
        .map(row => ({
            player: row.player,
            rating: row.rating_post, // Keep as float
            change: row.rating_post - row.rating_pre, // Keep as float
            games: 1
        }));
...
// EXACTLY like Python round_type_from_id function
function roundTypeFromId(roundId) {
    if (typeof roundId !== 'string') {
        return "TR";
    }
    const token = roundId.split("-")[0].trim();
    const finalToken = token.split()[0] || "TR";
    const WEIGHTS = { "TR": 2, "CR": 1.5, "TN": 1.75 };
    return finalToken in WEIGHTS ? finalToken : "TR";
}
...
function displayResults(results) {
    ...
    // Display final ratings - keep as floats like Python
    ...
    if (historyTable) {
        ...
        if (tbody) {
            ...
            results.history.forEach(player => {
                ...
                row.innerHTML = `
                    <td>${player.player}</td>
                    <td>${player.rating.toFixed(6)}</td>
                    <td>${player.change >= 0 ? '+' : ''}${player.change.toFixed(6)}</td>
                    <td>${player.games}</td>
                `;
                tbody.appendChild(row);
            });
    ...
    // Display snapshots - EXACTLY like Python output
    ...
    if (snapshotsTable && results.snapshots && results.snapshots.length > 0) {
        ...
        if (tbody) {
            tbody.innerHTML = '';
            // Show ALL snapshots, one block for each round - EXACTLY like Python
            results.snapshots.forEach(snapshot => {
                // Add the snapshot header - EXACTLY like Python
                const headerRow = document.createElement('tr');
                headerRow.innerHTML = `<td colspan=\"4\" style=\"font-weight: bold; background: #f8fafc; padding: 10px; border-bottom: 1px solid #e5e7eb; font-family: monospace; white-space: pre;\">================================================================================\n${snapshot.label}\n================================================================================</td>`;
                tbody.appendChild(headerRow);
                
                // Add column headers
                const columnHeaderRow = document.createElement('tr');
                columnHeaderRow.innerHTML = `<td colspan=\"4\" style=\"font-weight: bold; font-family: monospace; padding: 5px;\">           player      rating</td>`;
                tbody.appendChild(columnHeaderRow);
                
                // Sort players by rating descending and show top 20 like Python output
                Object.entries(snapshot.ratings)
                    .sort(([,a], [,b]) => b - a)
                    .slice(0, 20) // Top 20 like Python output shows
                    .forEach(([player, rating], index) => {
                        const row = document.createElement('tr');
                        const paddedPlayer = player.padEnd(16, ' ');
                        const formattedRating = rating.toFixed(6).padStart(12, ' ');
                        row.innerHTML = `
                            <td colspan=\"4\" style=\"font-family: monospace; padding: 2px 5px;\">${paddedPlayer}${formattedRating}</td>
                        `;
                        tbody.appendChild(row);
                    });
                ...
                spacerRow.innerHTML = '<td colspan=\"4\" style=\"height: 30px;\"></td>';
